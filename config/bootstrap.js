...
module.exports.bootstrap = async function () {
  ...
  const ryanDaht = await User.create({
  ...
  const jeannotDia = await User.create({
    emailAddress: "diambujeannot@gmail.com",
    fullName: "Diambu Jeannot",
    isSuperAdmin: true,
    password: await sails.helpers.passwords.hashPassword("jean123"),
  }).fetch();

  await User.create({
    emailAddress: "beni@gmail.com",
    fullName: "Sendo Beni",
    isSuperAdmin: true,
    password: await sails.helpers.passwords.hashPassword("beni"),
  }).fetch();

  // Add Ryan as one of jeannotDia is friends
  await User.addToCollection(jeannotDia.id, "friends", ryanDaht.id);

  //Probably not real life
  // await User.addToCollection(ryanDaht.id, "friends", jeannotDia.id);
